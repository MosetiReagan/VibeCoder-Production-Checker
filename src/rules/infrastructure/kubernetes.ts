...
export const kubernetesRiskySettings = createRule({
  ...
  description:
    'Finds privileged containers, host networking, hostPath volumes, and missing resource limits in Kubernetes manifests.',
  ...
  async run(context) {
    ...
    for (const file of context.files) {
      ...
      if (!/kind\s*:\s*(?:Pod|Deployment|StatefulSet|DaemonSet|Job|CronJob)/.test(file.content))
        continue;
        ...
        if (/privileged\s*:\s*true/i.test(line) || /hostNetwork\s*:\s*true/i.test(line)) {
          findings.push(
            createFinding({
              ...
              impact:
                'The workload can bypass normal container isolation and affect the host network or kernel.',
              recommendation:
                'Remove privileged mode and hostNetwork unless a documented runtime requirement exists. Apply least-privilege security contexts and network policies.'
            })
          );
        }
        if (/type\s*:\s*HostPath/i.test(line)) {
          findings.push(
            createFinding({
              ...
              recommendation:
                'Use PersistentVolumeClaims, ConfigMaps, Secrets, or object storage instead of HostPath.'
            })
          );
        }
      });
      if (!/resources\s*:/.test(file.content) || !/limits\s*:/.test(file.content)) {
        findings.push(
          createFinding({
            ...
            impact:
              'Runaway workloads can exhaust node CPU or memory and destabilize other services.',
            recommendation:
              'Set CPU and memory requests and limits based on measured workload usage.'
          })
        );
      }
    }
