...
export const dockerRootUser = createRule({
  ...
  async run(context) {
    ...
    for (const file of context.files.filter((item) => item.relativePath === 'Dockerfile')) {
      ...
      const resolvesToRoot = lastUser?.match?.[1]
        ?.toLowerCase()
        .match(/(?:^|[^a-z0-9])(?:root|0(?::0)?)(?:$|[^a-z0-9])/);
      if (!lastUser || resolvesToRoot)
        findings.push(
          createFinding({
            ruleId: this.id,
            title: this.title,
            ...
            impact:
              'A compromised application process has unnecessary root privileges inside the container.',
            recommendation:
              'Create an application user, set ownership only where needed, and end the runtime stage with USER appuser.'
          })
        );
    }
    return findings;
  ...
  async run(context) {
    ...
    for (const file of context.files) {
      ...
      file.lines.forEach((line, index) => {
        if (
          isCompose &&
          (/privileged\s*:\s*true/i.test(line) || /\/var\/run\/docker\.sock/.test(line))
        ) {
          findings.push(
            createFinding({
              ruleId: this.id,
              ...
              impact:
                'The container can effectively control the host Docker daemon and compromise the host.',
              recommendation:
                'Remove privileged mode and Docker socket mounts. Use a constrained CI service or rootless Docker when needed.'
            })
          );
        }
        if (isCompose && /network_mode\s*:\s*['"]?host/i.test(line)) {
          findings.push(
            createFinding({
              ...
              impact:
                'Container network isolation is weakened and services can reach every interface on the host.',
              recommendation:
                'Use bridge networking and publish only required ports to loopback or an internal network.'
            })
          );
        }
...
export const databaseExposed = createRule({
  ...
  async run(context) {
    ...
    for (const file of context.files.filter((item) =>
      /^docker-compose\.ya?ml$|^compose\.ya?ml$/.test(item.relativePath)
    )) {
      ...
      const services =
        (
          document as {
            services?: Record<
              string,
              { ports?: Array<string | number | { published?: string | number }> }
            >;
          }
        )?.services ?? {};
      for (const [serviceName, service] of Object.entries(services)) {
        for (const port of service.ports ?? []) {
          ...
          if (hostPort !== null && databasePorts.has(hostPort))
            findings.push(
              createFinding({
                ruleId: this.id,
                title: this.title,
                ...
                impact:
                  'The database can be reachable from host interfaces and potentially the public network.',
                recommendation:
                  'Remove the host port mapping and use an internal Compose network. For local access, bind to 127.0.0.1 and never expose production databases.'
              })
            );
        }
      }
...
function isLoopbackBinding(port: ComposePort): boolean {
  const value =
    typeof port === 'object' && port !== null ? String(port.published ?? '') : String(port);
