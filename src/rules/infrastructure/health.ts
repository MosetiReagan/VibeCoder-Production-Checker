...
export const missingHealthChecks = createRule({
  ...
  async run(context) {
    ...
    const dockerHasCheck = Boolean(dockerfile?.lines.some((line) => /^\s*HEALTHCHECK\s/.test(line)));
    ...
    return [createFinding({
      ...
      file: dockerfile?.relativePath ?? 'package.json',
      line: line === undefined || line === -1 ? 1 : line + 1,
      evidence: hasRoute ? 'Health endpoint exists but Docker HEALTHCHECK is missing' : 'No health endpoint found',
      description: 'The scanner did not find a complete, visible readiness health mechanism.',
      impact: 'Orchestrators may route traffic to unhealthy instances or fail to restart hung processes.',
      recommendation: 'Add liveness and readiness endpoints and configure the deployment or Docker HEALTHCHECK to use them.'
    })];
  }
});
