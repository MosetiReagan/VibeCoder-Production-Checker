    ...
    return scanLines(
      ...
      (file, line, text) => {
        ...
        return createFinding({
          ...
          description:
            'A localhost or loopback URL is configured for a production deployment path.',
          impact:
            'The application may call itself or a local service that does not exist in the production runtime.',
          recommendation:
            'Use an explicitly validated production hostname or service-discovery name. Reserve localhost for local development configuration.'
        });
      }
    );
  }
