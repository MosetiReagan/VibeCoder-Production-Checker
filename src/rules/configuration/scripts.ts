...
export const productionScripts = createRule({
  ...
  async run(context) {
    ...
    if (applicationDependencies.some((dependency) => dependencies[dependency]))
      required.push('start');
    ...
    return [
      createFinding({
        ...
        description:
          'The package manifest lacks scripts needed for reproducible production workflows.',
        impact:
          'Deployment, verification, and quality gates become dependent on undocumented commands.',
        recommendation:
          'Define build, start, test, and lint commands in package.json and use them consistently in local and CI workflows.'
      })
    ];
  }
});
