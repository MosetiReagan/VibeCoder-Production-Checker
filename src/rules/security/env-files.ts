...
export const envFilesTracked = createRule({
  ...
  async run(context) {
    ...
    for (const file of context.files.filter((item) =>
      /(?:^|\/)\.env(?:\..+)?$/.test(item.relativePath)
    )) {
      const fileName = path.posix.basename(file.relativePath);
      const ignored = context.gitignore.some(
        (line) =>
          line === fileName ||
          line === '.env*' ||
      ...
      if (!ignored && fs.existsSync(gitDir)) {
        ...
        findings.push(
          createFinding({
            ...
            description:
              'An environment file exists in a Git repository and is not covered by the observed ignore patterns.',
            impact:
              'Production credentials may be committed and shared with every repository reader.',
            recommendation:
              'Add .env* to .gitignore, remove tracked environment files with git rm --cached, and rotate exposed credentials. Keep a .env.example template without secrets.'
          })
        );
      }
    }
