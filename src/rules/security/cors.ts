...
export const wildcardCors = createRule({
  ...
  confidence: 'medium',
  async run(context) {
    const usesCredentials = context.files.some(
      (file) =>
        file.content.includes('credentials: true') ||
        file.content.includes('Access-Control-Allow-Credentials')
    );
    return scanLines(
      context,
      [
      ...
      (file, line, text) =>
        createFinding({
          ...
          recommendation:
            'Use an explicit allow-list of trusted origins. Avoid wildcard origins when cookies, Authorization headers, or credentials are enabled.'
        })
    );
  }
});
