    ...
    for (const file of context.files) {
      ...
      file.lines.forEach((text, index) => {
        const isAuthenticationRoute =
          /['"`/]?(?:\/login|\/register|\/auth|\/password-reset|\/forgot-password|\/otp)['"`/]?/.test(
            text
          );
        ...
        if (!hasLocalRateLimit) {
          findings.push(
            createFinding({
              ...
              description:
                'An authentication-related endpoint was found without rate limiting visible nearby.',
              impact:
                'Attackers can perform credential stuffing, brute force, OTP guessing, or resource exhaustion.',
              recommendation:
                'Apply rate limiting and lockout controls to authentication, password reset, OTP, and registration routes. This may be enforced by an upstream gateway.'
            })
          );
        }
      });
