...
export const insecureCookies = createRule({
  ...
  confidence: 'medium',
  async run(context) {
    return scanLines(context, [/cookie\s*\([^)]*(?:token|session|auth)/i, /res\.cookie\s*\(/], (file, line, text) => {
      ...
      return createFinding({
        ...
        impact: 'Session cookies can be exposed through transport interception, JavaScript access, or cross-site request patterns.',
        recommendation: 'Set Secure, HttpOnly, and SameSite=Lax or Strict. Use a strict setting for sensitive cookies and document intentional development exceptions.'
      });
    });
  }
});
