...
import { findUnsafeCommandExecutions, isJavaScriptLike } from '../analysis/typescript-ast.js';
...
export const commandInjection = createRule({
  ...
  async run(context) {
    const astFindings = context.files.filter(isJavaScriptLike).flatMap((file) =>
      findUnsafeCommandExecutions(file).map((finding) =>
        createFinding({
          ruleId: this.id,
          title: this.title,
    ...
    const legacyFindings = scanLines(
      legacyContext,
      dynamicCommandPatterns,
      (file, line, text, match) => {
